...
protected:
	...
	struct DroneRecord
	{
		...
		DroneRecord(unsigned int newDroneID, unsigned int newRange,
					...
					string newManufacturer, string newDescription, string newBatteryType) : prev(NULL), next(NULL), droneID(newDroneID), range(newRange),
...
public:
	...
	bool any_duplicates(DroneRecord value) const;
